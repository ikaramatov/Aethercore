...
class BlackjackGame(QWidget):
    def __init__(self, send_comment_callback=None):
        super().__init__()
        self.setWindowTitle("Blackjack with Aether")
    ...
    def split_hand(self):
        if len(self.hands) == 1 and self.get_card_value(self.hands[0][0]) == self.get_card_value(self.hands[0][1]):
            ...
            if self.send_comment_callback:
                self.send_comment_callback("Splitting, are we? Twice the loss potential~")
    ...
    def resolve_round(self):
        ...
        if self.send_comment_callback:
            self.send_comment_callback(f"Results — {results}. I had {dealer_score}.")
    ...
    def close_game(self):
        if self.send_comment_callback:
            self.send_comment_callback("Come again soon, player~")
        self.close()
